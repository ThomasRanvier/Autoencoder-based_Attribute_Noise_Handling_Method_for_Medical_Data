"""
Credit: https://github.com/BorisMuzellec/MissingDataOT/blob/master/imputers.py
"""
...
def nanmean(v, *args, **kwargs):
    ...
    v[is_nan] = 0
    return (v.sum(*args, **kwargs) / (~is_nan).float().sum(*args, **kwargs)).type(torch.cuda.FloatTensor)
...
class OTimputer():
    ...
    def fit_transform(self, X, verbose=True, report_interval=500, X_true=None):
        ...
        X = torch.from_numpy(X).clone().type(torch.cuda.FloatTensor)
        n, d = X.shape
        ...
        if self.batchsize > n // 2:
            ...
            self.batchsize = 2**e
            if verbose:
                print(f"Batchsize larger that half size = {len(X) // 2}. Setting batchsize to {self.batchsize}.")

        mask = torch.isnan(X).type(torch.cuda.FloatTensor)#.double()
        imps = ((self.noise * torch.randn(mask.shape).type(torch.cuda.FloatTensor) + nanmean(X, 0))[mask.type(torch.cuda.BoolTensor)]).type(torch.cuda.FloatTensor) #on mask .double()
